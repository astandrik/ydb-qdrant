...
- One‑table layout:
  - Global points table: `qdrant_all_points`
    ...
    - **Note**: Vector indexes are not supported in one‑table mode. Searches use a two‑phase flow: (1) approximate candidate selection over `embedding_bit` using the user's distance metric family, then (2) exact re‑ranking over `embedding` with the configured distance metric.
