...
export async function bootstrapMetaTable(): Promise<void> {
    ...
    await withSession(async (s) => {
        ...
        try {
            ...
            const td = new TableDescription()
                .withColumns(
                    ...
                    new Column("user_uid", Types.optional(Types.UTF8))
                )
        ...
        const hasLastAccessedAt = cols.some(
            (c) => c.name === "last_accessed_at"
        );
        ...
        if (!hasUserUid) {
            throw new Error(
                "bootstrapMetaTable: qdr__collections exists but is missing required column user_uid"
            );
        }
    });
}

function isBootstrapMetaTableEntrypoint(argv1: string | undefined): boolean {
    if (!argv1) {
        return false;
    }
    const entry = basename(argv1);
    return (
        entry === "bootstrapMetaTable.ts" || entry === "bootstrapMetaTable.js"
    );
}
