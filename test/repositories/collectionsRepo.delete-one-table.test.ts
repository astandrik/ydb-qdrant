...
function createDescribeTableMock() {
    return vi.fn((tableName: string) => {
        if (tableName === "qdrant_points_by_file") {
...
describe("collectionsRepo/deleteCollection one-table (with mocked YDB)", () => {
    ...
    it("deletes points from global table in one_table mode", async () => {
        ...
        expect(querySessionMock.execute).toHaveBeenCalledTimes(2);
        const executeCall = querySessionMock.execute.mock.calls[0];
        expect(executeCall).toBeDefined();
        const executeArgs = executeCall?.[0];
        expect(executeArgs?.text).toContain(
            "BATCH DELETE FROM qdrant_all_points"
        );
        expect(executeArgs?.text).toContain("WHERE collection = $collection");
        const lookupCleanupCall = querySessionMock.execute.mock.calls[1]?.[0];
        expect(lookupCleanupCall?.text).toContain(
            "BATCH DELETE FROM qdrant_points_by_file"
        );
